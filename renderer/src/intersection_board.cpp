...
void IntersectionTile::addRectangle(int4 ltrb, int16_t groupIndex)
{
    ...
    assert(groupIndex >
           simd::reduce_max(findMaxIntersectingGroupIndex(ltrb, 0)));
    ...
    assert(simd::all(ltrb <
                     255)); // Ensure ltrb isn't completely outside the tile.
    ...
    // m_edges is a list of 8 rectangles encoded as [L, T, 255 - R, 255 - B],
    // relative to m_topLeft. The data is also transposed: [L0..L7, T0..T7,
    // -R0..R7, -B0..B7]. Bias ltrb by -128 so we can use int8_t. SSE doesn't
    // have an unsigned byte compare.
    int4 biased = ltrb + std::numeric_limits<int8_t>::min();
    m_edges.back()[subIdx] = biased.x;
    m_edges.back()[subIdx + 8] = biased.y;
    m_edges.back()[subIdx + 16] =
        biased.z; // Already converted to "255 - right" above.
    m_edges.back()[subIdx + 24] =
        biased.w; // Already converted to "255 - bottom" above.
...
int16x8 IntersectionTile::findMaxIntersectingGroupIndex(
    int4 ltrb,
    int16x8 runningMaxGroupIndices) const
{
    ...
    // Since we mask non-intersecting groupIndices to zero, the "mask and max"
    // algorithm is only correct for positive values. (runningMaxGroupIndices is
    // only signed because SSE doesn't have an unsigned max instruction.)
    ...
    if (simd::all(ltrb == 255))
    {
        ...
        runningMaxGroupIndices[0] =
            std::max(runningMaxGroupIndices[0], m_maxGroupIndex);
    ...
    // the right.
    //
    // Bias ltrb by -128 so we can use int8_t. SSE doesn't have an unsigned byte
    // compare.
    int4 biased = ltrb + std::numeric_limits<int8_t>::min();
    int8x8 r = biased.z;
    int8x8 b = biased.w;
    ...
    __m128i localMaxGroupIndices =
        math::bit_cast<__m128i>(runningMaxGroupIndices);
    ...
    runningMaxGroupIndices[0] =
        std::max(runningMaxGroupIndices[0], m_baselineGroupIndex);
    return runningMaxGroupIndices;
}
...
int16_t IntersectionBoard::addRectangle(int4 ltrb, int16_t layerCount)
{
    ...
    for (int y = span.y; y <= span.w; ++y)
    {
        ...
        {
            maxGroupIndices =
                tileIter->findMaxIntersectingGroupIndex(ltrb, maxGroupIndices);
            ++tileIter;
        }
    }
